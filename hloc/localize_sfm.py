...
def pose_from_cluster(
    ...
    num_matches = 0
    points3D = localizer.reconstruction.points3D
    for i, db_id in enumerate(db_ids):
        ...
        matches = matches_result[pair]["matches"]
        idx = np.where(matches != -1)[0]
        matches = np.stack([idx, matches[idx]], -1)
        # matches_0, _ = get_matches(matches_path, qname, image.name)
    ...
    # kp_loc = [np.array([kpq[i][0], kpq[i][1]], dtype=np.float64) for i in idxs for _ in kp_idx_to_3D[i]]
    # kp_3d_loc = [points3D[j].xyz for i in idxs for j in kp_idx_to_3D[i]]
    # kp_3d_loc = [np.array([[points3D[j].xyz[0]], [points3D[j].xyz[1]], [points3D[j].xyz[2]]], dtype=np.float64) for i in idxs for j in kp_idx_to_3D[i]]
    kp_loc = np.array([[kpq[i][0], kpq[i][1]] for i in idxs for _ in kp_idx_to_3D[i]], dtype=np.float64)
    kp_3d_loc = np.array([points3D[j].xyz for i in idxs for j in kp_idx_to_3D[i]], dtype=np.float64)
    answer = pycolmap.absolute_pose_estimation(kp_loc, kp_3d_loc, query_camera, estimation_options=dict(ransac=dict(max_error=12.0)), refinement_options=dict(refine_focal_length=True))
    # answer = pycolmap.pose_refinement(quat2matrix)
    ...
    return ret, log, 'answer', kp_loc, kp_3d_loc
